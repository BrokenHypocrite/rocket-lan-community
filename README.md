...
**Pre-release.** Custom-map LAN nights for Rocket League, with friends, over the internet.

This repository is the **community-facing hub** for Rocket LAN. The app itself
is a separate, closed-source project; here you'll find:
...
Rocket LAN lets you and friends play Rocket League **custom maps** together
without anyone setting up a real LAN, port-forwarding their router, or
fighting Windows networking. It spins up a private virtual network on each
player's machine, then lets Rocket League's own built-in LAN match mode do
the rest.
...
- **EAC-compatible.** Works whether you launch with Easy Anti-Cheat on or off.
  Rocket LAN does not inject, hook, or patch the game.
...
Rocket LAN is in **pre-release**. There's no public download link yet. The
[roadmap page](https://BrokenHypocrite.github.io/rocket-lan-community/roadmap.html)
on the site tracks what's shipped, what's in flight, and what's planned —
and the Discussions tab here is where we collect feedback from early testers.
